...
class Qlearning:
    ...
    def visualization(self):
        ...
        if self.mode_mouse:
            plt_mouse = np.array([4,4])
        else:
            plt_mouse = self.env.mouse
        map = self.env.world  #[plt_mouse[0]-6:plt_mouse[0]+6, plt_mouse[1]-6:plt_mouse[1]+6]
        print(map.shape)
        #map[self.env.mouse[0]][self.env.mouse[1]] = 2   # mouse
        cMap = plt.cm.get_cmap('PuBuGn')#('GnBu')#('YlOrRd')
        plt.pcolormesh(map, cmap=cMap, edgecolor=(0,0,0),linewidths=0.1,linestyles='dashed')
        #plt.axis(xlim=[0,self.height],ylim=[0,self.width])
        
        plt.axis('equal')
        for x in range(self.width): #range(plt_mouse[1]-6,plt_mouse[1]+6): 
            for y in range(self.height): #range(plt_mouse[0]-6, plt_mouse[0]+6): 
                ...
                if x==plt_mouse[1] and y==plt_mouse[0]:
                    ...
                    #plt.plot(x-plt_mouse[1]+6+0.5,y-plt_mouse[0]+6+0.5,'*',markersize=100/np.sqrt(self.height))
                    ...
                    #plt.quiver(x-plt_mouse[1]+6+0.5-action[1]*0.25,y-plt_mouse[0]+6+0.5-action[0]*0.25,action[1],action[0],(0,0,1), width=0.08/self.height,scale=2,scale_units='y')
